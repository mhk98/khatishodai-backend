...
const updateOneFromDB = async (id, payload) => {
  const { subCategoryItemTitle, category_id, subCategory_id } = payload;
  const data = {
    subCategoryItemTitle:
      subCategoryItemTitle === undefined ? undefined : subCategoryItemTitle,
    category_id: category_id === undefined ? undefined : category_id,
    subCategory_id: subCategory_id === undefined ? undefined : subCategory_id,
  };
  const result = await SubCategoryItem.update(data, {
    where: {
...
const deleteOneFromDB = async (id) => {
  const result = await SubCategoryItem.destroy({
    where: {
      subCategoryItemId: id,
    },
  });
